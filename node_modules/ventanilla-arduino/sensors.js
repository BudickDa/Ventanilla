...
exports.registerSensor = function(block,inputs,cb){
  if(initFunctions[block.name]!==undefined){
    for(i in inputs){
      console.log("Register Sensor on ArduinoBoard " + inputs[i]);
      var input = blocks[inputs[i]];
...
function tryRegister(block, input, cb){
  if(!blocks[input].isReady){
    console.log("Board is not ready... waiting.");
    return setTimeout(function(){
      return tryRegister(block, input, cb)}
    ,1500);
  }else{
