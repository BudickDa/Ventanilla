...
function sendData(block){
  ...
  try{
    if(block.input!==undefined){
      for(i in block.input){
        if(blocks[block.input[i].uid]===undefined||blocks[block.input[i].uid].value===undefined){
          console.log("Error in sendData in vetnanilla/sendData: Block does not exists.");
          continue;
        }
        request.post({
          url:"http://"+block.hardware.ip + "/api",
          json:{"value":blocks[block.input[i].uid].value,"key":block.hardware.key,"name":block.input[i].type}
        }, function(error,response,body){
            if(error){console.log("Error sending Api-input: "+error);}
    ...
    try{ //here the existence of the block should be tested...
        setTimeout(function(){
          return sendData(block);
        },1000);
      }catch(e){
